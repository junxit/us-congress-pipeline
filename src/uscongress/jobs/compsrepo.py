...
history would mean holding all 633 MB in memory per commit. :meth:`GitRepo.sync_tree`
writes only what changed, which is the same reason the US Code build uses it:
consecutive snapshots differ by a handful of packages out of ~2,700.
"""
...
def seed(repo_path: Path | None = None) -> GitRepo:
    ...
    manifest_path = repo.path.parent / f".{repo.path.name}.manifest.json"
    for day, path in pending:
        files, failed = _files_for(path)
        # Recorded in the tree so that a day on which nothing changed still
        # produces a commit. Without it `sync_tree` writes nothing, `commit`
        # finds a clean tree and returns False, and the day leaves no trace --
        ...
        change = repo.sync_tree(files, manifest_path=manifest_path)
        note = f", {failed} package(s) govinfo would not serve" if failed else ""
        repo.commit(
            f"Statute Compilations — {day.isoformat()}\n\n"
            f"{len(files):,} compilations, {change.written:,} changed and "
            f"{change.removed:,} withdrawn since the previous snapshot{note}.\n\n"
        ...
        print(
            f"  {day}: {len(files):,} compilations, "
            f"{change.written:,} changed, {change.removed:,} withdrawn",
            flush=True,
        )
    return repo
