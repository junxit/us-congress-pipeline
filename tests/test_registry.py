...
def test_every_phase_of_the_plan_is_present() -> None:
    """The plan has eleven phases, 0 through 10, and none may go missing.
...
def test_the_phases_that_produce_no_repository_are_the_expected_six() -> None:
    ...
    Standing up the loop, backfilling a family, derived material, votes, the
    credentials the loop needs to run unattended, and the members crosswalk that
    makes those votes joinable. If a seventh appears, either the plan changed or
    a `produces` was dropped by accident; both are worth stopping on.
    """
    assert [p.number for p in PHASES if not p.produces] == [3, 4, 7, 8, 9, 10]
...
def test_the_roadmap_explains_the_gap_in_the_numbering() -> None:
    ...
    """
    text = "\n".join(_roadmap())
    assert "Phases 3, 4, 7, 8, 9 and 10 produce no repository" in text
